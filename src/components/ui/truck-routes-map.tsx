...
// Inline truck silhouette (lucide-style), drawn at 24x24, centered at (0,0).
const TRUCK_SVG = (
  <g transform="translate(-12 -12)" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export function TruckRoutesMap({ pins, color = "oklch(0.38 0.14 18)" }: TruckRoutesMapProps) {
  const hq = pins.find((p) => p.hq);
  const hqPt = hq ? project(hq.lat, hq.lng) : null;
  const others = pins.filter((p) => !p.hq);
  ...
  return (
    <div className="relative w-full">
      <svg
        ...
        role="img"
        aria-label="Mapa do Brasil com rotas de caminhão para as filiais"
      >
        <defs>
          <linearGradient id="trk-fill" x1="0" y1="0" x2="1" y2="1">
          ...
          <radialGradient id="trk-glow" cx="50%" cy="50%" r="50%">
          ...
          {hqPt &&
            others.map((p) => {
              ...
              return <path key={`def-${p.uf}`} id={`trk-path-${p.uf}`} d={d} />;
            })}
        </defs>

        <ellipse cx={VIEW_W / 2} cy={VIEW_H / 2} rx={VIEW_W / 2.2} ry={VIEW_H / 2.4} fill="url(#trk-glow)" opacity="0.25" />
        ...
        {/* Caminhões lentos animados ao longo de cada rota */}
        {hqPt &&
          others.map((p, i) => (
            <g key={`truck-${p.uf}`} color="white">
              <g>
                <rect x={-14} y={-12} width={28} height={24} rx={6} fill={color} opacity="0.95" />
                {TRUCK_SVG}
                <animateMotion
                  dur={`${14 + (i % 4) * 2}s`}
                  ...
                  begin={`${i * 0.8}s`}
                >
                  <mpath href={`#trk-path-${p.uf}`} />
                </animateMotion>
              </g>
            </g>
          ))}

        {/* Pinos das capitais (sem bolinha branca) */}
        {pins.map((p, i) => {
          ...
          return (
            ...
            >
              {p.hq && (
                <circle cx={pt.x} cy={pt.y} r={r * 2.2} fill={color} opacity="0.2">
                  <animate attributeName="r" values={`${r};${r * 3};${r}`} dur="2.4s" repeatCount="indefinite" />
                  <animate attributeName="opacity" values="0.4;0;0.4" dur="2.4s" repeatCount="indefinite" />
                </circle>
              )}
              <circle cx={pt.x} cy={pt.y} r={r} fill={color} stroke={color} strokeWidth="2" />
              <text
                x={pt.x + r + 6}
                y={pt.y + 4}
                fontSize={p.hq ? 16 : 13}
